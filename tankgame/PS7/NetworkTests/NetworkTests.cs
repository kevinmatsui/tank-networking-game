...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        public void toCall(SocketState state) // ^ for StopServer
        {
        }

        [TestMethod]
        public void StartServerErrorTest()
        {
            NetworkUtil.Networking.StartServer(toCall, 80);
            NetworkUtil.Networking.StartServer(toCall, 80);
        }
        ...
        {
            TcpListener tcp = NetworkUtil.Networking.StartServer(toCall, 2112);
            NetworkUtil.Networking.StopServer(tcp); 
        }

        [TestMethod]
        public void ConnectHostErrorTest()
        {
            NetworkUtil.Networking.ConnectToServer(toCall, "random", 80);
        }

        [TestMethod]
        public void BeginConnectTest()
        {
            NetworkUtil.Networking.ConnectToServer(toCall, "localhost", -1000);
        }

        [TestMethod]
        public void SendAndCloseTest1()
        {
            IPAddress ip = IPAddress.Parse("127.0.0.1");
            Socket socket = new Socket(ip.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
            socket.Close();
            Assert.IsFalse(NetworkUtil.Networking.SendAndClose(socket,"data"));
        }

        [TestMethod]
        public void BeginSendTestException()
        {
            IPAddress ip = IPAddress.Parse("127.0.0.1");
            Socket socket = new Socket(ip.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
            Assert.IsFalse(NetworkUtil.Networking.Send(socket, null));
        }

        [DataRow(true)]
        [DataRow(false)]
        [DataTestMethod]
        public void SendAndCloseTest(bool clientSide)
        {
